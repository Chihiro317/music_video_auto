...
def _write_test_audio(path: Path, duration: float = 4.0, sample_rate: int = 44100) -> None:
    ...
    # Add a pulse-like tone so beat detection has stronger transients.
...
def _draw_dance_frame(path: Path, phase: float, turn: bool = False) -> None:
    ...
    # Arms and legs move strongly to make dance visible.
...
def _write_test_background(path: Path) -> None:
    ...
    frame[:, :, 0] = (12 + 75 * x + 35 * y).clip(0, 255).astype(np.uint8)
    frame[:, :, 1] = (6 + 28 * y).clip(0, 255).astype(np.uint8)
    frame[:, :, 2] = (75 + 160 * (1 - y) + 50 * x).clip(0, 255).astype(np.uint8)
